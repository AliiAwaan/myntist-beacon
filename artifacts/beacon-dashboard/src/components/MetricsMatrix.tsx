...
export default function MetricsMatrix({ records, loading }: Props) {
  const rows = (records || []).slice(0, 7);

  return (
    <div className="bg-[#1F3864] rounded-xl p-5 border border-[#0f1c38] h-full">
      ...
      {loading && rows.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs uppercase tracking-widest text-gray-500 border-b border-[#0f1c38]">
            ...
            <tbody>
              {rows.map((r: any, i: number) => (
                <tr key={r.id || i} className="border-b border-[#0f1c38]/50 hover:bg-white/5 transition-colors">
                  <td className="py-2 px-2 font-mono text-gray-400 text-xs">{formatTs(r.recorded_at)}</td>
                  ...
                  <td className="py-2 px-2 font-mono text-right text-blue-300">{r.Q?.toFixed(3) ?? "—"}</td>
                  <td className="py-2 px-2 font-mono text-right text-purple-300">{r.tau?.toFixed(4) ?? "—"}</td>
