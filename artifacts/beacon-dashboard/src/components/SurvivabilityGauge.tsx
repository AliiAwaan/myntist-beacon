...
export default function SurvivabilityGauge({ latest, loading }: Props) {
  const S: number | null = latest?.S ?? null;
  const delta_S: number = latest?.delta_S ?? 0;
  const Q: number | null = latest?.Q ?? null;
  const tau: number | null = latest?.tau ?? null;
      ...
      <div className="flex flex-col items-center">
        <div className="relative">
          ...
          <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}>
            <path d={trackPath} fill="none" stroke="#0f1c38" strokeWidth={STROKE} strokeLinecap="round" />
            {fillPath && (
            ...
            <text x={CX} y={CY + 14} textAnchor="middle" fill="#9ca3af" fontSize="10" fontFamily="sans-serif">
              S SCORE
            </text>
        ...
        <div className="grid grid-cols-2 gap-3 w-full mt-4">
          <div className="bg-[#0f1c38] rounded-lg p-3 text-center">
            ...
            <div className={`text-lg font-mono font-bold ${delta_S > 0 ? "text-green-400" : delta_S < 0 ? "text-red-400" : "text-gray-400"}`}>
              {delta_S > 0 ? "▲" : delta_S < 0 ? "▼" : "—"} {Math.abs(delta_S).toFixed(4)}
            </div>
          </div>
          <div className="bg-[#0f1c38] rounded-lg p-3 text-center">
