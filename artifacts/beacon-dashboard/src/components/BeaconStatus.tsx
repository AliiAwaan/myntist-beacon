...
function stateColor(S: number | null) {
...
function stateTextColor(S: number | null) {
...
export default function BeaconStatus({ latest, loading, error, secondsUntilRefresh }: Props) {
  const S: number | null = latest?.S ?? null;
  const field_state: string = latest?.field_state ?? "unknown";
  const timestamp: string | null = latest?.timestamp ?? null;

  return (
    <div className="bg-[#1F3864] border-b border-[#0f1c38] px-6 py-3">
      <div className="max-w-screen-2xl mx-auto flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="text-center">
            ...
            ) : secondsUntilRefresh === 0 && error ? (
              <div className="flex items-center gap-1.5">
                <span className="inline-block w-2 h-2 rounded-full bg-amber-400 animate-ping" />
                <span className="text-sm font-mono text-amber-400">retrying…</span>
              </div>
            ) : secondsUntilRefresh === 0 ? (
              <div className="flex items-center gap-1.5">
                <span className="inline-block w-2 h-2 rounded-full bg-green-400 animate-ping" />
                <span className="text-sm font-mono text-green-400">refreshing…</span>
              </div>
            ) : (
              <div className="flex items-center gap-1.5">
                <span className="inline-block w-2 h-2 rounded-full bg-indigo-400 animate-pulse" />
                <span className="text-sm font-mono text-indigo-300">{secondsUntilRefresh}s</span>
              </div>
            )}
            <div className="text-xs text-gray-400 uppercase tracking-widest mt-0.5">Next Refresh</div>
          </div>
        </div>
      </div>
    </div>
