...
class FinancialEngine:
    ...
    def compute_coherence_signal(self, tau: float, q_variance: float) -> float:
        """
        Coherence signal Ttau, normalised to [0.0, 1.0].
        Formula: min(1.0, tau / q_variance)
        ...
        The min(1.0, ...) clamp ensures the value never exceeds 1.0 regardless
        of how small q_variance becomes above the epsilon floor.
        """
        if q_variance < self._COHERENCE_MIN_VARIANCE:
            return 0.0
        return min(1.0, tau / q_variance)
