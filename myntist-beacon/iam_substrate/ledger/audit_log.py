"""
Capital Formation Audit Ledger

Records every identity event, policy decision, role
assignment, and capital-forming mechanism execution
in an append-only audit log backed by PostgreSQL.

Capital-forming outputs (float_yield, r_HSCE,
liquidity_signal, coherence_signal) are computed
by the financial engine and accessible via
/telemetry/finance. This ledger captures the
event history that drives those computations.

Implements the audit trail requirement from
IAM_SUBSTRATE_SPEC_v1.3.2 §12.
"""
