...
Phase 2 endpoints:
  ...
  GET  /rss.xml              → RSS 2.0 feed of recent beacon pulses (Criterion B46)
...
from identity_loop.feeds.rss_generator import generate_rss as _generate_rss
...
def rss_feed() -> Response:
    """RSS 2.0 feed of recent beacon pulses (Criterion B46)."""
    REQUEST_COUNT.labels(method="GET", path="/rss.xml").inc()
    import os as _os
    try:
        rows = _exporter.get_recent_field_telemetry(days=7)
    except Exception:
    ...
    items = []
    for r in rows:
        raw_ts = r.get("time", r.get("timestamp", None))
        ...
        items.append({
            ...
            "field_state": r.get("field_state", "unknown"),
            "timestamp": ts_str,
        })
    tmp_path = "/tmp/myntist_rss_feed.xml"
    _generate_rss(items, output_path=tmp_path)
...
def trigger_score(req: ScoreRequest, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    _s_before: Optional[float] = float(identity.S) if identity else None
    ...
    if not identity:
        ...
        identity.delta_S = result.S - _s_before
    ...
    try:
        from iam_substrate.ledger.audit_log import append_audit_entry as _append_audit
        _append_audit(
            db=db,
            identity_id=req.identity_id,
            event_type="score",
            action=f"score computed: S={result.S:.4f} field_state={result.field_state}",
            S_before=_s_before,
            S_after=result.S,
        )
    except Exception as _audit_exc:
        logger.warning("audit entry failed (POST /score): %s", _audit_exc)
